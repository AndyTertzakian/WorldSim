...
public class BreatheFireCommand implements Command {
	...
	private int range = 5 ;
	private ArenaAnimal livingItem;

	public BreatheFireCommand(Direction direction, ArenaAnimal animal) {
	...
	public void execute(World world) throws InvalidCommandException {
		...
		for (Item i : targets) {
			if (Util.getDirectionTowards(livingItem.getLocation(), i.getLocation()).equals(xDirection)
					&& livingItem.getLocation().getDistance(i.getLocation()) <= range && !i.getName().equals("Charizard")) {
